...
struct App
{
	...
	double minimalAverageStepTime=1e+60;
	...
	int enterApp()
	{
		...
		wnd.renderHandler( [&](SoftwareRenderer& r)
		{
            ...
            if(frame_count == 200)
            {
				double averageStepTime=frame_time / frame_count;
				if (averageStepTime < minimalAverageStepTime) {minimalAverageStepTime=averageStepTime;}
                std::cout << "Average step time: " << averageStepTime << " ms\t(Minimal so far: " << minimalAverageStepTime << " ms)" << std::endl;
                frame_time = 0.0;
                frame_count = 0;
            }
		});
...
//------------------------------------------------------
template<typename T>
struct MyTable2D : public Table2D<T> {
    template<typename F>
	void parallel_fill2(F&& f)
	{
        static const int n_threads = 1;
...
struct MyApp : public App {
	std::array<MyTable2D<char>, 2> table;
};


int main(int, char**) {
	//return App{}.enterApp();
	return MyApp{}.enterApp();
}
